...
class Cell{
	...
	public:
		...
		//Control DOF:
		char getControl(int i) const {return Control_[i];}
...
void verletalgo(Cell& cell,std::vector<Particle>& ps, double dt){
	...
	h = h + hd*dt + hdd0 * 0.5 * dt_2;

	//Calcul des positions particules fin du pas de temps

	for(std::vector<Particle>::iterator it = ps.begin(); it != ps.end(); it++){
		//Force pas de temps precedent entre particules
		//+ force imposee par BCU
		Vecteur F = it->getForce(); 
		Vecteur r = it->getR();
		Vecteur v = it->getV();
		double m = it->getMasse();

		//Acceleration imposee par cellule:
		//Pour le moment les calculs sont faits meme si on
		//sait qu'ils ne contribuent pas (ForceCel nul)
		//A optimiser plus tard:
		Vecteur ForceCel = hinv * r;
		ForceCel = TotalStress * ForceCel; 
		ForceCel = hinv * ForceCel;
		ForceCel = ForceCel * V0;
		//a0 a stocker
		Vecteur a0 = F * (1./m) + ForceCel * (1./mh); 
		r = r + v * dt + a0 * dt_2 * 0.5;
	...
	for(std::vector<Particle>::iterator it = ps.begin(); it != ps.end(); it++){
		...
		tmp = hinv * tmp;
		tmp = tmp * (V1/mh);
		//tmp est nul suivant direction controlee en vitesse
